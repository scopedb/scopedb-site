...
SQL has been standing the test of time for 50 years, as the primary language for structured data processing. However, SQL is also an old language with significant design problems, especially on its non-composable and inconsistent syntax.

When developing the query language for ScopeQL, we decided to go against SQL and design a new language, ScopeQL, from scratch to fix SQL's problems. Nevertheless, we recognized the goodness of relational algebra and decided to implement it in ScopeQL.

In this post, we will discuss where SQL falls short and the design principles behind ScopeQL.
...
The first and probably most significant problem is that, **the syntactic order of the query above is not the semantic order**. The query above would be actually interpreted in the following order:
...
When subqueries are used, the mismatch between the syntactic order and the semantic order becomes even more apparent. The subquery is evaluated first, and then the outer query is evaluated on the result of the subquery. Typical analytical queries can have multiple levels of subqueries, which makes the query even harder to read and understand.
...
As a preview, equivalent logic can be expressed sequentially in ScopeQL:
...
SQL has an original goal to have a ["walk up and read"](https://dl.acm.org/doi/10.1145/3555041.3589336) property. And thus, SQL chooses a pseudo-English syntax. Unfortunately, this decision creates **a lot of rigid and arbitrary syntax, as well as too many keywords**.
...
To sum up, all these syntax issues combine to make SQL difficult to learn, and excessively difficult to work with, even for SQL experts.
...
However, in ScopeQL, you can simply write:

```scopeql
FROM foo GROUP BY x + 1 AS x2 AGGREGATE MAX(y) AS y_max
```

You may notice that the `GROUP BY` is referred to as a modifier, not as a clause. This is because it's only a modifier of an `AGGREGATE` clause or a `WINDOW` clause, and thus, you always need to specify an aggregate function or window function after `GROUP BY`:
...
No more `OVER` and `PARTITION BY` keywords specifically for window functions.

To achieve the same functionality as the SQL query with bare `GROUP BY`, you can use the `DISTINCT ON` clause:
